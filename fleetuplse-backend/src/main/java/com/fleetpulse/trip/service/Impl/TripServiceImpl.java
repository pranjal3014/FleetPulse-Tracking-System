...
public class TripServiceImpl implements TripService {
	...
	public TripResponse saveTrip(TripRequest request) {
		...
		boolean driverAssigned = tripRepository.existsByDriverDriverIdAndTripStatusIn(driver.getDriverId(),
				activeStatuses);

		if (driverAssigned) {

			throw new DriverAlreadyAssignedException("Driver is already assigned to another trip");
		}

		boolean vehicleAssigned = tripRepository.existsByVehicleVehicleIdAndTripStatusIn(vehicle.getVehicleId(),
				activeStatuses);

		if (vehicleAssigned) {

			throw new VehicleAlreadyAssignedException("Vehicle is already assigned to another trip");
		}
	...
	public RouteDetails getTripRoute(Long tripId) {
		...
		return routeProvider.getRoute(trip.getPickupLocation(), trip.getDestinationLocation());
	}

	private TripResponse convertToDto(Trip trip) {
		return TripResponse.builder().tripId(trip.getTripId()).drivername(trip.getDriver().getDriverName())
				.vehicleNumber(trip.getVehicle().getVehicleNumber()).pickupLocation(trip.getPickupLocation())
				.destinationLocation(trip.getDestinationLocation()).tripDate(trip.getTripDate())
				.tripTime(trip.getTripTime()).tripStatus(trip.getTripStatus()).createdAt(trip.getCreatedAt()).build();
	}
}
